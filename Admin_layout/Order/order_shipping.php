...
$totalPage = ceil(mysqli_num_rows(mysqli_query($connect, "SELECT * FROM danhsach_donhang WHERE Status = '$stt_dh' AND Phone LIKE ('$textFix')")) / $pageRow);
$listPrd = "";
$x = 1;
$y = 2;
...
for ($i = 1; $i <= $totalPage; $i++) {
    ...
    $prevPg = '
        <li class="page-item">
            <a class="page-link" 
            href="'.$configHref.'/admin/trangquantri.php?Admin=order_shipping&s=' . $text . '&page_filter=' . $x . '"
            aria-label="Next">
    ...
    $listPrd .= '<li class="page-item">
            ...
            href="'.$configHref.'/admin/trangquantri.php?Admin=order_shipping&s=' . $text . '&page_filter=' . $i . '">' . $i . '</a>
        </li>';
    $nextPg = '<li class="page-item"><a class="page-link" href="'.$configHref.'/admin/trangquantri.php?Admin=order_shipping&s=' . $text . '&page_filter=' . $y . '" aria-label="Next"><span aria-hidden="true">Next</span></a></li>';
}

if ($hasSearch) {
    $sql = "SELECT * FROM danhsach_donhang WHERE Status = '$stt_dh' AND Phone LIKE ('$textFix') ORDER BY id_dh DESC LIMIT $perRow,$pageRow";
} else {
    $sql = "SELECT * FROM danhsach_donhang WHERE Status = '$stt_dh' ORDER BY updated_at LIMIT $perRow,$pageRow";
}
...
<form action="<?= $configHref ?>/admin/trangquantri.php" method="get" class="mt-5" style="max-width: 460px; margin: auto" >
    <div class="input-group mb-3">
        <input type="hidden" name="Admin" value="order_shipping">
